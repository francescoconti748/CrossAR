...
SNR = [ 40, 35, 30, 25, 20, 15]; %we re-produce results for this case
%SNR = [10];
...
poles_x = "[ 0.7, pi/12; 0.7, -pi/12; 0.7, pi/4; 0.7, -pi/4]"; %explicit poles as string vectors [modules_vector, phases_vector]
poles_y = "[ 0.7, pi/12; 0.7, -pi/12; 0.7, pi/4; 0.7, -pi/4]"; %exp1
%poles_y = "[ 0.7, pi/12+pi/8; 0.7, -pi/12-pi/8; 0.7, pi/4+pi/8; 0.7, -pi/4-pi/8]"; %exp2
...
Pe_Burg = zeros( nRuns, n_SNR);
Pe_Cross = zeros( nRuns, n_SNR);
Px_signal = zeros( nRuns, n_SNR);
...
for idx_snr = 1:length(SNR)
    ...
    set_param('CrossAR/ModelParameters','SNR_dB', num2str(SNR(idx_snr)))
    sim("CrossAR.slx" )
    ...
    figure( "Name", ['Channel X: SNR: ' num2str(SNR(idx_snr))] );
    ...
    for idx_run = 1:nRuns
        ...
        RMSE_Burg_x(idx_run, idx_snr  ) = sqrt( sum((a_x - ax_Burg(idx_run, :)).^2)/(N+1) ) ;
        RMSE_CrossB_x(idx_run, idx_snr  ) = sqrt( sum(( a_x - ax_crossB(idx_run, :)).^2)/(N+1)  );
        
        RMSE_Burg_y(idx_run, idx_snr  ) = sqrt( sum((a_y - ay_Burg(idx_run, :)).^2)/(N+1) ) ;
        RMSE_CrossB_y(idx_run, idx_snr  ) = sqrt( sum(( a_y - ay_crossB(idx_run, :)*(flip( eye(N+1) )) ).^2)/(N+1)  );
...
%         scatter( real(roots_cross_y), imag(roots_cross_y), 'g.' )
        ...
        Pe_Burg(idx_run, idx_snr  ) = sum((x_gt - x_est_Burg).^2)/length(w_run);
        Pe_Cross(idx_run, idx_snr  ) = sum((x_gt - x_est_Cross).^2)/length(w_run);   
        Px_signal(idx_run, idx_snr) = sum((x_gt).^2)/length(w_run);      
...
data = { [RMSE_Burg_x(:,1), RMSE_Burg_x(:, 2), RMSE_Burg_x(:, 3), RMSE_Burg_x(:,4), RMSE_Burg_x(:,5)], ...
   [RMSE_CrossB_x(:,1), RMSE_CrossB_x(:, 2), RMSE_CrossB_x(:, 3), RMSE_CrossB_x(:,4), RMSE_CrossB_x(:,5) ] };
figure
boxplotGroup( data, 'PrimaryLabels', {'Burg', 'Cross'}, ...
   'SecondaryLabels', {['SNR = ' num2str(SNR(1))], ['SNR = ' num2str(SNR(2))], ...
      ['SNR = ' num2str(SNR(3))], ['SNR = ' num2str(SNR(4))], ['SNR = ' num2str(SNR(5))]}, ...  
...
%
datay = { [RMSE_Burg_y(:,1), RMSE_Burg_y(:, 2), RMSE_Burg_y(:, 3), RMSE_Burg_y(:,4), RMSE_Burg_y(:,5) ], ...
   [RMSE_CrossB_y(:,1), RMSE_CrossB_y(:, 2), RMSE_CrossB_y(:, 3), RMSE_CrossB_y(:,4) , RMSE_CrossB_y(:,5)] };
figure
...
%%
%data = { [Pe_Burg(:,1), Pe_Burg(:, 2), Pe_Burg(:, 3), Pe_Burg(:,4)], ...
%    [Pe_Cross(:,1), Pe_Cross(:, 2), Pe_Cross(:, 3), Pe_Cross(:,4) ] };
%figure
%boxplotGroup( data, 'PrimaryLabels', {'Burg', 'Cross'}, ...
%    'SecondaryLabels', {['SNR = ' num2str(SNR(1))], ['SNR = ' num2str(SNR(2))], ...
